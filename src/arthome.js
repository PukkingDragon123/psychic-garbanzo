...
(function (PD) {
  ...
  /* ====================================================================== A PERSON
     Built from the feet up, in this order, because every layer needs the one
     under it to be already there: tail, legs, torso, clothes, arms, neck,
     head, face, hair, and whatever he went out wearing.

     The face is the part that matters. An eye here is a socket, a white, an
     iris that is never the colour of his skin, a pupil, a catchlight that is
     always up-and-left, and a lid that cuts the top off the white -- six
     elements, on a head big enough to hold them. Everything above the eyes is
     brow and everything below is moustache, and between them that is a
     character rather than a colour swatch with legs. */
  function buildAlien(t, face) {
    const HW = (t.hr * 2 + 34) & ~1;
    const HH = (t.hr * 2 + t.th + t.lh + 34) & ~1;
    const W = HW, H = HH;
    const p = pix(W, H);
    const cx = W >> 1;
    const FOOT = H - 2;
    ...
    const shY = hipY - t.th;
    const neckY = shY - 2;
    const headY = neckY - t.hr + 1;                  // the centre of the head
    const INK = '#140f26';
    const kiss = face === 4, blink = face === 5, cheer = face === 6;
    const talk = face === 3 || cheer;
    const step = face === 1 ? 1 : (face === 2 ? -1 : 0);

    // ------------------------------------------------------------- the tail
    if (t.plan === 'beast') {
      const sweep = step * 2;
      let tx = cx - t.hw + 1, ty = hipY - 4;
      for (let i = 0; i < 6; i++) {
        const w = 6 - i * 0.7;
      ...
      for (const side of [-1, 1]) {
        ...
        p.round(cx + side * (t.sw - 1) - 3, shY + 4, 7, t.th + 1, 3, t.acc);
      }
    }
    ...
    // ------------------------------------------------------------ the torso
    // a proper taper from shoulder to hip instead of a rectangle
    for (let y = 0; y <= t.th + 3; y++) {
      const f = y / (t.th + 3);
      const wAt = Math.round(t.sw + (t.hw - t.sw) * f);
      p.rect(cx - wAt, shY + y, wAt * 2, 1, t.skin);
    }
    p.round(cx - t.sw, shY - 1, t.sw * 2, 6, 3, t.skin);          // shoulder cap
    p.round(cx - t.sw + 1, shY, t.sw * 2 - 2, 4, 2, t.lite);      // and the light on it
    p.round(cx - t.hw + 1, shY + t.th, t.hw * 2 - 2, 5, 2, t.dark);
    if (t.build === 'stout' || t.build === 'hulk' || t.build === 'pear') {
      p.ellipse(cx, shY + t.th - 1, t.hw - 1, 5, t.skin);
      p.ellipse(cx, shY + t.th - 2, t.hw - 3, 3, t.lite);
    }

    // --------------------------------------------------------- the clothing
    // nothing is a black slab any more: every garment leaves a collar of skin
    // at the neck and has a lit edge, so the body reads in three tones
    const C = t.cloth, CL = t.clothL, CD = t.clothD;
    const colY = shY + 3;
    if (t.wear === 'vest') {
      p.round(cx - t.sw, shY + 1, 6, t.th + 3, 2, C);
      p.round(cx + t.sw - 6, shY + 1, 6, t.th + 3, 2, C);
      p.rect(cx - t.sw, shY + 1, 6, 1, CL); p.rect(cx + t.sw - 6, shY + 1, 6, 1, CL);
      for (let i = 0; i < 3; i++) p.set(cx + t.sw - 3, shY + 6 + i * 4, t.acc);
    } else if (t.wear === 'jacket') {
      p.round(cx - t.sw, colY, t.sw * 2, t.th + 1, 3, C);
      p.round(cx - 4, colY, 8, t.th + 1, 2, t.lite);              // shirt under it
      p.poly = null;
      p.round(cx - t.sw, colY - 2, 7, 8, 2, CL);                  // lapels
      p.round(cx + t.sw - 7, colY - 2, 7, 8, 2, CL);
      p.rect(cx - t.sw, colY + t.th - 2, t.sw * 2, 2, CD);
      p.rect(cx - 1, colY + 3, 2, t.th - 5, t.acc);               // a tie
      p.round(cx - 2, colY + 1, 5, 4, 1, t.acc);
    } else if (t.wear === 'tank') {
      p.round(cx - t.sw + 2, colY + 1, t.sw * 2 - 4, t.th, 2, C);
      p.rect(cx - t.sw + 2, colY + 1, t.sw * 2 - 4, 1, CL);
      p.rect(cx - t.sw + 2, colY + 5, t.sw * 2 - 4, 1, CD);
    } else if (t.wear === 'sash') {
      for (let i = 0; i < t.th + 2; i++) {
        p.rect(cx - t.sw + 1 + Math.round(i * 0.7), colY + i, 6, 1, i % 4 < 2 ? C : CL);
      }
    } else if (t.wear === 'coat') {
      p.round(cx - t.sw - 1, colY, t.sw * 2 + 2, t.th + 6, 3, C);
      p.rect(cx - 1, colY, 2, t.th + 6, CD);
      p.round(cx - t.sw - 1, colY - 2, 7, 9, 2, CL);
      p.round(cx + t.sw - 6, colY - 2, 7, 9, 2, CL);
      for (let i = 0; i < 3; i++) p.set(cx + 2, colY + 5 + i * 4, t.acc);
    } else if (t.wear === 'sequin') {
      p.round(cx - t.sw, colY, t.sw * 2, t.th + 2, 2, C);
      for (let y = colY + 1; y < colY + t.th + 2; y += 2) {
      ...
      p.round(cx - t.sw, colY - 2, 6, 7, 2, CL);
      p.round(cx + t.sw - 6, colY - 2, 6, 7, 2, CL);
      for (let i = 0; i < 3; i++) p.set(cx, colY + Math.round(t.th * 0.7) + i * 3, t.acc);
    }
    // a waist, always, so the body is not one shape
    p.rect(cx - t.hw + 1, shY + t.th + 2, t.hw * 2 - 2, 3, t.pantsD);
    p.rect(cx - t.hw + 1, shY + t.th + 2, t.hw * 2 - 2, 1, t.pants);
    if (t.belt) {
      p.rect(cx - t.hw, shY + t.th + 1, t.hw * 2, 3, CD);
      p.round(cx - 3, shY + t.th, 6, 5, 1, t.acc);
      p.set(cx, shY + t.th + 2, t.accD);
    }
    if (t.chain) {
      for (let i = -3; i <= 3; i++) {
        p.set(cx + i * 2, colY + 4 + Math.abs(i), t.acc);
        p.set(cx + i * 2 + 1, colY + 4 + Math.abs(i), t.accD);
      }
    ...
    p.rect(cx - 3, neckY - 3, 7, 2, t.skin);
    ...
    if (t.plan === 'fungal') {
      ...
      p.round(cx - Math.round(HR * 0.7), headY - 4, Math.round(HR * 1.4), HR + 9, 5, '#fff6e0');
      p.ellipse(cx, headY - 8, cw, ch, t.dark);                    // the gills
      for (let i = -5; i <= 5; i++) p.line(cx + i * 2, headY - 8, cx + i * 3, headY - 8 - ch, t.skin);
      p.ellipse(cx, headY - 11, cw, ch, t.skin);                   // and the cap over them
      p.ellipse(cx, headY - 13, Math.round(cw * 0.78), Math.round(ch * 0.66), t.lite);
      for (let i = 0; i < 6; i++) {
        const a = 0.5 + i * 0.7;
        p.disc(cx + Math.cos(a) * cw * 0.6, headY - 13 + Math.sin(a) * ch * 0.4,
          1.4 + (i % 2), i % 2 ? '#fff6e0' : t.acc);
      }
      p.rect(cx - cw, headY - 8, cw * 2, 1, t.dark);
    } else if (t.plan === 'beast') {
      p.ellipse(cx, headY, HR, HR - 1, t.skin);
      p.ellipse(cx, headY - Math.round(HR * 0.42), Math.round(HR * 0.76), Math.round(HR * 0.36), t.lite);
      const sd = t.lean > 0 ? 1 : -1;
      const snY = headY + Math.round(HR * 0.28);
      p.round(cx + sd * (HR - 3) - 3, snY - 3, t.snout + 7, Math.round(HR * 0.62), 4, t.skin);
      p.round(cx + sd * (HR - 3) - 2, snY - 3, t.snout + 5, Math.round(HR * 0.3), 3, t.lite);
      p.round(cx + sd * (HR + t.snout - 1) - 2, snY - 1, 5, 4, 2, t.dark);      // the nose
    ...
    if (t.plan === 'bug') {
      ...
      p.round(cx - t.sw + 2, shY - 1, t.sw * 2 - 4, 3, 1, t.acc);
    }
    ...
    if (t.tex === 'speckle') {
      for (let i = 0; i < 14; i++) {
        const a = i * 1.7;
        p.set(cx + Math.cos(a) * HR * 0.62, headY + Math.sin(a) * HR * 0.55, t.dark);
        p.set(cx + Math.cos(a * 1.3) * t.sw * 0.6, shY + 5 + (i % Math.max(1, t.th)), t.dark);
      }
    } else if (t.tex === 'scale') {
      for (let y = shY + 4; y < shY + t.th; y += 3) {
        for (let x = cx - t.sw + 2; x < cx + t.sw - 1; x += 4) {
          p.set(x + ((y / 3) | 0) % 2 * 2, y, t.dark);
        }
      }
      for (let y = headY - 2; y < headY + HR - 3; y += 3) p.rect(cx - HR + 3, y, 2, 1, t.dark);
    } else if (t.tex === 'stripe') {
      for (let i = 0; i < 4; i++) {
        p.rect(cx - t.sw + 1, shY + 4 + i * 4, t.sw * 2 - 2, 1, t.dark);
        p.rect(cx - HR + 4, headY - HR + 5 + i * 3, (HR - 4) * 2, 1, t.dark);
      }
    } else if (t.tex === 'spot') {
      for (let i = 0; i < 5; i++) {
        const a = i * 1.9 + t.seed % 3;
        p.ellipse(cx + Math.cos(a) * HR * 0.5, headY + Math.sin(a) * HR * 0.45, 2 + (i % 2), 2, t.dark);
      }
    } else if (t.tex === 'glow') {
      for (let i = 0; i < 4; i++) {
        for (const sd of [-1, 1]) {
          p.set(cx + sd * Math.round(HR * 0.72), headY - 4 + i * 3, t.glowCol);
          p.set(cx + sd * (t.sw - 2), shY + 5 + i * 3, t.glowCol);
        }
      }
    } else if (t.tex === 'plate') {
      p.round(cx - HR + 3, headY - HR + 3, (HR - 3) * 2, 6, 3, t.lite);
    }
    ...
    /* ONE EYE. A socket so it sits in the head, a white, an iris that is never
       ...
       lid cutting the top off the white. Six things, and any one of them
       missing and it goes back to being a dot. */
    function eye(ex, ey, r, shut) {
      const rx = Math.max(2, Math.round(r)), ry = Math.max(2, Math.round(r * 1.16));
      p.ellipse(ex, ey, rx + 1, ry + 1, t.dark);
      if (shut) {
        p.rect(ex - rx, ey, rx * 2 + 1, 2, INK);
        p.rect(ex - rx + 1, ey - 2, rx * 2 - 1, 2, t.skin);
        for (let k = -1; k <= 1; k++) p.set(ex + k * Math.round(rx * 0.7), ey + 2, t.dark);
        return;
      }
      p.ellipse(ex, ey, rx, ry, '#f6f8ff');
      p.ellipse(ex, ey + 1, rx, Math.max(1, ry - 1), '#ffffff');
      const ix = ex + Math.round(t.lean * rx * 0.3);
      const iy = ey + Math.round(ry * 0.16);
      p.ellipse(ix, iy, Math.max(2, Math.round(rx * 0.66)), Math.max(2, Math.round(ry * 0.66)), t.iris);
      p.ellipse(ix, iy + 1, Math.max(1, Math.round(rx * 0.5)), Math.max(1, Math.round(ry * 0.4)), t.irisL);
      p.ellipse(ix, iy, Math.max(1, Math.round(rx * 0.34)), Math.max(1, Math.round(ry * 0.42)), INK);
      p.rect(ix - Math.round(rx * 0.62), iy - Math.round(ry * 0.56), 2, 2, '#ffffff');
      p.set(ix + Math.round(rx * 0.44), iy + Math.round(ry * 0.34), '#ffffff');
      // the lid, in skin, over the top quarter
      p.ellipse(ex, ey - ry - 1, rx + 1, Math.max(1, Math.round(ry * 0.72)), t.skin);
      p.rect(ex - rx, ey - ry, rx * 2 + 1, 1, t.dark);
    }

    /* And a brow over it, which is where all the mood lives. */
    function brow(ex, ey, r, side) {
      const w = Math.round(r * 2.1), th2 = t.brow === 'bushy' ? 4 : 2;
      const top = ey - Math.round(r * 1.9);
      let tilt = 0;
      if (t.brow === 'angry') tilt = side * 2;
      else if (t.brow === 'sad') tilt = -side * 2;
      else if (t.brow === 'arch') tilt = -1;
      const col = t.hair === 'none' ? t.dark : t.hairC;
      for (let i = 0; i < w; i++) {
        const f = i / Math.max(1, w - 1);
        const yy = top + Math.round(tilt * (side > 0 ? f : 1 - f)) - (t.brow === 'arch' ? Math.round(Math.sin(f * Math.PI) * 1.5) : 0);
        p.rect(ex - Math.round(w / 2) + i, yy, 1, th2, col);
      }
    }

    const pos = [];
    if (t.eyeK === 'stack') for (let i = 0; i < t.eyes; i++) pos.push([cx, faceY - Math.round(er * 1.2) + i * (er * 2 + 2)]);
    else if (t.eyeK === 'tri') pos.push([cx - gap, faceY - 2], [cx + gap, faceY - 2], [cx, faceY + er + 3]);
    else if (t.eyeK === 'stalk') {
      for (let i = 0; i < t.eyes; i++) {
        const sxp = cx + (t.eyes === 1 ? 0 : (i ? gap : -gap));
        p.rect(sxp - 2, headY - HRr - 8, 4, 12, t.dark);
        p.rect(sxp - 2, headY - HRr - 8, 1, 12, t.lite);
        pos.push([sxp, headY - HRr - 9]);
      }
    } else if (t.eyes === 1) pos.push([cx, faceY]);
    else for (let i = 0; i < t.eyes; i++) pos.push([cx + (i - (t.eyes - 1) / 2) * (gap * 2), faceY]);

    const shut = kiss || blink;
    const bigEye = t.eyes <= 2 ? er : Math.max(3, Math.round(er * 0.72));
    for (let i = 0; i < pos.length; i++) {
      const q = pos[i];
      eye(Math.round(q[0]), Math.round(q[1]), bigEye, shut);
    }
    if (!shut && t.eyeK !== 'stalk' && t.eyes <= 2) {
      for (let i = 0; i < pos.length; i++) {
        brow(Math.round(pos[i][0]), Math.round(pos[i][1]), bigEye, pos.length === 1 ? 1 : (i ? 1 : -1));
      }
    }

    // a nose, because a face without one is a mask
    const noseY = faceY + Math.round(er * 1.6);
    if (t.plan !== 'beast' && t.plan !== 'bug') {
      p.round(cx - 2, noseY - 1, 5, 4, 2, t.dark);
      p.round(cx - 2, noseY - 2, 5, 3, 2, t.skin);
      p.set(cx - 2, noseY + 1, t.dark); p.set(cx + 2, noseY + 1, t.dark);
    }

    // ------------------------------------------------------------ the mouth
    const my = faceY + Math.round(HRr * 0.56);
    if (kiss) { p.ellipse(cx, my, 4, 3, '#ff5fa8'); p.ellipse(cx, my - 1, 2, 2, '#ffd6f0'); }
    else if (talk) {
      p.ellipse(cx, my, 5, 4, '#3a1226');
      p.rect(cx - 4, my - 2, 9, 1, '#ffffff');
      p.rect(cx - 3, my + 2, 7, 1, '#c4566f');
    } else if (t.mouth === 'grin') {
      p.rect(cx - 5, my, 11, 1, INK);
      p.set(cx - 6, my - 1, INK); p.set(cx + 6, my - 1, INK);
      p.rect(cx - 3, my + 1, 7, 1, t.dark);
    } else if (t.mouth === 'smirk') {
      p.rect(cx - 4, my, 9, 1, INK);
      p.set(cx + 5, my - 1, INK); p.set(cx + 6, my - 2, INK);
    } else if (t.mouth === 'lips') { p.ellipse(cx, my, 5, 2, t.accD); p.rect(cx - 4, my - 1, 9, 1, INK); }
    else if (t.mouth === 'tusk') {
      p.rect(cx - 5, my, 11, 1, INK);
      p.spike(cx - 4, my - 6, 4, 7, -1, '#ffffff'); p.spike(cx + 5, my - 6, 4, 7, -1, '#ffffff');
    } else if (t.mouth === 'beak') { p.spike(cx, my - 3, 9, 8, 1, t.acc); p.rect(cx - 5, my - 3, 11, 1, INK); }
    else if (t.mouth === 'straw') { p.rect(cx - 1, my, 2, 9, t.dark); p.disc(cx, my + 9, 2, t.acc); }
    else if (t.mouth === 'mandible') {
      p.spike(cx - 6, my - 2, 5, 8, 1, t.dark); p.spike(cx + 6, my - 2, 5, 8, 1, t.dark);
    ...
    // -------------------------------------------------------- facial hair
    // the single cheapest way to turn a generated body into somebody you
    // would recognise again across a crowded room
    const HC = t.hairC, HL = t.hairL;
    if (t.hair === 'walrus') {
      p.round(cx - 8, my - 4, 17, 6, 2, HC);
      p.round(cx - 8, my - 4, 17, 2, 2, HL);
      p.round(cx - 10, my - 3, 5, 8, 2, HC);
      p.round(cx + 6, my - 3, 5, 8, 2, HC);
      p.rect(cx - 2, my - 4, 5, 2, t.dark);
    } else if (t.hair === 'handlebar') {
      p.round(cx - 7, my - 4, 15, 4, 1, HC);
      for (const sd of [-1, 1]) {
        p.round(cx + sd * 8 - 2, my - 5, 4, 4, 1, HC);
        p.round(cx + sd * 10 - 2, my - 8, 4, 4, 2, HC);
        p.set(cx + sd * 11, my - 9, HL);
      }
    } else if (t.hair === 'pencil') {
      p.rect(cx - 6, my - 3, 13, 2, HC);
      p.rect(cx - 6, my - 3, 13, 1, HL);
    } else if (t.hair === 'tache') {
      p.round(cx - 6, my - 4, 13, 4, 2, HC);
      p.round(cx - 6, my - 4, 13, 2, 2, HL);
    } else if (t.hair === 'goatee') {
      p.round(cx - 5, my - 4, 11, 3, 1, HC);
      p.round(cx - 4, my + 2, 9, 7, 3, HC);
      p.round(cx - 3, my + 3, 7, 3, 2, HL);
    } else if (t.hair === 'beard') {
      p.round(cx - 6, my - 4, 13, 4, 2, HC);
      p.round(cx - HRr + 3, my - 1, (HRr - 3) * 2, HRr - 1, 5, HC);
      p.round(cx - HRr + 5, my, (HRr - 5) * 2, 4, 3, HL);
      p.ellipse(cx, my, 5, 3, '#3a1226');
      for (let i = -2; i <= 2; i++) p.set(cx + i * 3, my + HRr - 3, HL);
    } else if (t.hair === 'chops') {
      for (const sd of [-1, 1]) {
        p.round(cx + sd * (HRr - 3) - 3, faceY - 2, 7, HRr - 1, 3, HC);
        p.round(cx + sd * (HRr - 3) - 2, faceY - 1, 5, 4, 2, HL);
      }
    }
    ...
    if (t.crown === 'fin') {
      ...
      for (const s of [-1, 1]) {
        p.spike(cx + s * (HRr - 3), crownY - 7, 7, 11, -1, t.lite);
        p.round(cx + s * (HRr - 4) - 2, crownY - 1, 5, 5, 2, t.lite);
      }
    } else if (t.crown === 'antenna') {
      for (let i = 0; i < t.ant; i++) {
        const ax = cx + (i - (t.ant - 1) / 2) * 7;
        const hgt = 8 + ((t.seed >> (i + 2)) & 5);
        p.rect(ax - 1, crownY - hgt, 3, hgt + 4, t.dark);
        p.rect(ax - 1, crownY - hgt, 1, hgt + 4, t.lite);
        p.disc(ax, crownY - hgt, 2.5 + (i & 1), t.acc);
        p.set(ax - 1, crownY - hgt - 1, '#ffffff');
      }
    } else if (t.crown === 'tuft' || t.crown === 'quiff') {
      const n = t.crown === 'quiff' ? 3 : 5;
      p.round(cx - HRr + 3, crownY - 1, (HRr - 3) * 2, 7, 4, HC);
      for (let i = 0; i < n; i++) {
        const sx2 = cx - (n - 1) * 3 + i * 6;
        p.round(sx2 - 3, crownY - 7 - (t.crown === 'quiff' ? 4 : 0), 7, 9, 3, HC);
        p.round(sx2 - 2, crownY - 6 - (t.crown === 'quiff' ? 4 : 0), 4, 4, 2, HL);
      }
    } else if (t.crown === 'ears') {
      for (const s of [-1, 1]) {
        p.ellipse(cx + s * (HRr + 1), headY - 3, 4, 7, t.skin);
        p.ellipse(cx + s * (HRr + 1), headY - 3, 2, 4, t.dark);
        p.ellipse(cx + s * (HRr + 1), headY - 4, 1, 2, t.lite);
      }
    } else if (t.crown === 'hat') {
      p.round(cx - 10, crownY - 10, 21, 11, 3, '#2a2438');
      p.rect(cx - 15, crownY - 1, 31, 3, '#2a2438');
      p.rect(cx - 10, crownY - 5, 21, 2, t.acc);
      p.rect(cx - 15, crownY - 1, 31, 1, '#453c5c');
    } else if (t.crown === 'cap') {
      p.round(cx - 11, crownY - 8, 23, 10, 4, t.clothD);
      p.rect(cx - 11, crownY - 1, 23, 3, t.cloth);
      p.round(cx + t.lean * 8 - 9, crownY + 1, 18, 4, 2, t.clothD);
      p.round(cx - 3, crownY - 10, 6, 3, 1, t.acc);
    } else if (t.crown === 'mohawk') {
      for (let i = 0; i < 6; i++) {
        const hgt = 14 - Math.abs(i - 1) * 2;
        p.spike(cx - 7 + i * 3, crownY - hgt + 2, 5, hgt, -1, i % 2 ? t.acc : t.accD);
      }
      p.rect(cx - 8, crownY, 17, 2, t.accD);
    } else if (t.crown === 'tendrils') {
      for (let i = 0; i < 5; i++) {
        const sd = i % 2 ? 1 : -1, off = (i - 2) * 4;
        let px2 = cx + off, py2 = crownY + 1;
        for (let k = 0; k < 5; k++) {
          p.rect(px2 - 1, py2, 3, 3, k > 2 ? t.acc : t.dark);
          px2 += sd * (k > 1 ? 1 : 0); py2 -= 3;
        }
        p.disc(px2, py2, 2, t.acc);
      }
    } else if (t.crown === 'crest') {
      const sd = t.lean > 0 ? -1 : 1;
      for (let i = 0; i < 13; i++) {
        const w = Math.max(1, 11 - i * 0.78);
        p.rect(cx + sd * i * 1.25 - w / 2, crownY - 1 - i, w, 2, i < 3 ? t.skin : t.lite);
      }
      p.rect(cx - 6, crownY - 1, 13, 2, t.dark);
    } else if (t.crown === 'bald') {
      p.ellipse(cx, headY - Math.round(HRr * 0.5), Math.round(HRr * 0.5), 3, t.lite);
    }

    // ------------------------------------------------- and what he went out in
    if (t.shades === 'visor') {
      const vw = gap + er + 2;
      p.round(cx - vw, faceY - er - 1, vw * 2, er * 2 + 2, 3, '#1b1430');
      p.round(cx - vw + 1, faceY - er, vw * 2 - 2, er + 1, 2, '#2e2450');
      p.rect(cx - vw + 1, faceY - er, vw * 2 - 2, 1, t.acc);
      p.rect(cx - vw + 3, faceY - er + 2, 4, 2, '#ffffff');
      p.rect(cx - HRr, faceY - 1, 3, 3, t.accD); p.rect(cx + HRr - 3, faceY - 1, 3, 3, t.accD);
    } else if (t.shades === 'specs') {
      for (const sd of [-1, 1]) {
        p.ellipse(cx + sd * gap, faceY, er + 2, er + 2, '#241b3a');
        p.ellipse(cx + sd * gap, faceY, er + 1, er + 1, 'rgba(0,0,0,0)');
        p.ellipse(cx + sd * gap, faceY - 1, Math.round(er * 0.6), 2, '#c9bce8');
      }
      p.rect(cx - 2, faceY, 5, 2, t.acc);
      p.rect(cx - HRr, faceY - 1, 3, 2, t.acc); p.rect(cx + HRr - 2, faceY - 1, 3, 2, t.acc);
    }
    if (t.smoke) {
      const sdx = t.lean > 0 ? 1 : -1;
      p.rect(cx + Math.min(sdx * 5, sdx * 14), my + 2, 10, 3, '#5a3a22');
      p.rect(cx + Math.min(sdx * 5, sdx * 14), my + 2, 10, 1, '#7a5230');
      p.rect(cx + sdx * 14, my + 2, 2, 3, '#ff7a2a');
      p.set(cx + sdx * 15, my + 3, '#ffe08a');
    }
    p.outline(P.ink);
    return p;
  }
